...
import Brightness4Icon from '@mui/icons-material/Brightness4'; import SpaIcon from '@mui/icons-material/Spa';
...
function Menu() {
    let { Color, setColor, setBackground, Border, setBorder, Home, setHome, Explore, setExplore, Notification, setNotification, Messages, setMessages, postBg, setpostBg, BaseColor, SetBaseColor } = MyStates();
    const SwitchEvent = (e) => {
    ...
    return (
        <div className="menu-container" style={{ borderRight: `${Border}`, position: "sticky", top: "1px" }}>
            <div className="logo">
            ...
            <div className="menu">
                <div className="menu-item" onClick={setHomeTrue} style={{ color: `${Home ? `${BaseColor}` : Color}` }}>
                    <HomeIcon /><Typography variant='p' fontWeight={600} >Home</Typography>
                </div>
                <div className="menu-item" onClick={setExploreTrue} style={{ color: `${Explore ? `${BaseColor}` : Color}` }}>
                    <ExploreIcon /><Typography variant='p' fontWeight={600}>Explore</Typography>
                </div>
                <div className="menu-item" onClick={setNotificationTrue} style={{ color: `${Notification ? `${BaseColor}` : Color}` }}>
                    <NotificationsIcon /><Typography variant='p' fontWeight={600}>Notifications</Typography>
                </div>
                <div className="menu-item" onClick={setMessagesTrue} style={{ color: `${Messages ? `${BaseColor}` : Color}` }}>
                    <MailOutlineIcon /><Typography variant='p' fontWeight={600}>Messages</Typography>
                </div>
            </div>
            <div className="Darkmode">
                <Brightness4Icon style={{ color: `${Color}` }} />
